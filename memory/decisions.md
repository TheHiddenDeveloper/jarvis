...
- [2026-08-07] Added 7 tools to jarvis-tools MCP (src/index.js), all read-only/local, no new deps: system_status (disk/mem/uptime/failed units/pacman -Qu), pkg_updates (read-only list), reminders (remind_add/remind_list/remind_clear/remind_fire — JSON store state/reminders.json, fires desktop+ntfy), web_search (DuckDuckGo html parse), web_fetch (Node fetch). Verified via direct MCP stdio calls. Daemon must restart to pick them up. Suggested next: email (needs msmtp) + calendar (needs khal/vdirsyncer) — NOT installed, ask before pacman install. Found smb.service (Samba) in failed state.
